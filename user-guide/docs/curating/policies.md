...
DDR enables publishing datasets subsequently within a project, and each dataset will bear a unique citation and DOI. This arises from the longitudinal and tiered structure of some research projects such as experiments and field research missions which may happen over time, may involve different teams, and require the publication of different documents or datasets.

While users may publish more than one dataset in a project at one time, subsequent publications are  enabled for Experimental, Simulation, Hybrid Simulation, and Field research data types/projects. Project types Other and Research Software do not allow subsequent publications.
