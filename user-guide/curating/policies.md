...
For each project type, the metadata elements including those that are required (\*), recommended (&), and generated by the repository system (!) are shown below.

Many fields are only recommended because users may not have information to fill them. For example, researchers may not have been awarded a grant to conduct research toward a dataset and therefore no information to input. We strongly encourage filling as much recommended metadata as possible so if you have been supported by a grant, please fill that information. Once you click and start filling a recommended field, corresponding sub-fields become required.
